...
<template>
    <WarehouseLayout :warehouses="warehouses" :navigation="navigation" :selectedWh="selectedWh" :currentBlock="'stockIn'">
        ...
        <div class="mt-2">
            <div class="grid grid-cols-7 gap-2 m-1 hover:bg-gray-100 rounded">
                <div>#</div>
                <div class="col-span-4">Название</div>
                <!--div>Цена (зак.)</div>
                <div>Цена</div-->
                <div>В наличии</div>
                <div title="В заказах, не списано со склада">Резерв</div>
            </div>
            <div v-for="(position, index) in stocks.data" v-key="index" class="grid grid-cols-7 gap-2 m-1 hover:bg-gray-100 rounded">
