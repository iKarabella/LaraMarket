...
const fChange = () => {
    useForm({filters:props.filters}).post(route('admin.warehouses.orders', currentWhInfo.value.code), {
        preserveScroll:true,
    });
};
</script>
            ...
            <div>
                <div v-for="order in orders.data" :key="order.id" class="hover:bg-gray-200 rounded-md mb-2">
                    <div class="grid grid-cols-6 gap-2">
                        ...
                        <div>
                            <Link :href="route('admin.warehouses.order', [currentWhInfo.code, order.uuid])"><SecondaryButton>Открыть</SecondaryButton></Link>
                        </div>
                    </div>
